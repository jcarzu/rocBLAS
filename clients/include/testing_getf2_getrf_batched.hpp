...
rocblas_status testing_getf2_getrf_batched(Arguments argus)
{
    ...
    if(M < 1 || N < 1 || lda < M || batch_count < 1)
    {
        ...
        auto         dIpiv_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(int)),
                                                rocblas_test::device_free};
        rocblas_int* dIpiv         = (rocblas_int*)dIpiv_managed.get();

        auto         dinfo_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(int)),
                                                rocblas_test::device_free};
        ...
        if(getrf)
        {
            return rocsolver_getrf_batched<T>(
                handle, M, N, dA, lda, dIpiv, stridep, dinfo, batch_count);
        }
        else
        {
            return rocsolver_getf2_batched<T>(
                handle, M, N, dA, lda, dIpiv, stridep, dinfo, batch_count);
        }
    }
    ...
    auto dIpiv_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(int) * size_piv),
                                            rocblas_test::device_free};
    rocblas_int* dIpiv = (rocblas_int*)dIpiv_managed.get();
    auto dinfo_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(int) * batch_count),
                                            rocblas_test::device_free};
    rocblas_int* dinfo = (rocblas_int*)dinfo_managed.get();
    ...
    if(argus.unit_check || argus.norm_check)
    {
        ...
        if(getrf)
        {
            CHECK_ROCBLAS_ERROR(rocsolver_getrf_batched<T>(
                handle, M, N, dA, lda, dIpiv, stridep, dinfo, batch_count));
        }
        else
        {
            CHECK_ROCBLAS_ERROR(rocsolver_getf2_batched<T>(
                handle, M, N, dA, lda, dIpiv, stridep, dinfo, batch_count));
        }
        ...
        for(int b = 0; b < batch_count; b++)
            CHECK_HIP_ERROR(
                hipMemcpy(hAr[b].data(), A[b], sizeof(T) * size_A, hipMemcpyDeviceToHost));
        CHECK_HIP_ERROR(
            hipMemcpy(hIpivr.data(), dIpiv, sizeof(int) * size_piv, hipMemcpyDeviceToHost));
        CHECK_HIP_ERROR(
            hipMemcpy(hinfor.data(), dinfo, sizeof(int) * batch_count, hipMemcpyDeviceToHost));
        ...
        if(getrf)
        {
            for(int b = 0; b < batch_count; ++b)
                cblas_getrf<T>(
                    M, N, hA[b].data(), lda, (hIpiv.data() + b * stridep), (hinfo.data() + b));
        }
        else
        {
            for(int b = 0; b < batch_count; ++b)
        ...
        for(int b = 0; b < batch_count; ++b)
        {
            ...
            {
                piverr = 1;
                cerr << "error singular pivot (batch " << b << "): " << hinfo[b] << " vs "
                     << hinfor[b] << endl;
            }
            ...
            for(int j = 0; j < min(M, N); j++)
            {
                ...
                {
                    piverr = 1;
                    cerr << "error reference pivot " << j << " (batch " << b << "): " << refPiv
                         << " vs " << gpuPiv << endl;
                    break;
                }
            }
    ...
    if(argus.timing)
    {
        ...
        cout << endl;
        cout << M << "," << N << "," << lda << "," << stridep << "," << gpu_time_used << ","
             << cpu_time_used;
