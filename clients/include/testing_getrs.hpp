...
rocblas_status testing_getrs(Arguments argus)
{
    ...
    if(M < 0 || nhrs < 0 || lda < std::max(1, M) || ldb < std::max(1, M))
    {
        ...
        auto dIpiv_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(int) * M),
                                                rocblas_test::device_free};
        rocblas_int* dIpiv = (rocblas_int*)dIpiv_managed.get();

        status = rocsolver_getrs<T>(handle, transRoc, M, nhrs, dA, lda, dIpiv, dB, ldb);
    ...
    if(argus.unit_check || argus.norm_check)
    {
        ...
        else
        {
            ...
            for(int i = 0; i < M; i++)
            {
                for(int j = 0; j < nhrs; j++)
                {
                    max_err_1 = max_err_1 > hBRes[i + j * ldb] ? max_err_1 : hBRes[i + j * ldb];
                }
            }
    ...
    if(argus.timing)
    {
        ...
        cout << M << " , " << nhrs << " , " << lda << " , " << ldb << " , " << gpu_time_used
             << " , " << cpu_time_used;
