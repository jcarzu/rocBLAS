...
rocblas_status testing_geqr2_geqrf_strided_batched(Arguments argus)
{
    ...
    if(M < 1 || N < 1 || lda < M || batch_count < 0)
    {
        ...
        if(geqrf)
        {
            return rocsolver_geqrf_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
        }
        else
        {
            return rocsolver_geqr2_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
        }
    }
    ...
    T*   dA            = (T*)dA_managed.get();
    auto dIpiv_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(T) * size_piv),
                                            rocblas_test::device_free};
    T*   dIpiv         = (T*)dIpiv_managed.get();
    ...
    if(argus.unit_check || argus.norm_check)
    {
        ...
        if(geqrf)
        {
            CHECK_ROCBLAS_ERROR(rocsolver_geqrf_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count));
        }
        else
        {
            CHECK_ROCBLAS_ERROR(rocsolver_geqr2_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count));
        }
        //copy output from device to cpu
        CHECK_HIP_ERROR(hipMemcpy(hAr.data(), dA, sizeof(T) * size_A, hipMemcpyDeviceToHost));
        CHECK_HIP_ERROR(
            hipMemcpy(hIpivr.data(), dIpiv, sizeof(T) * size_piv, hipMemcpyDeviceToHost));
        ...
        if(geqrf)
        {
            for(int b = 0; b < batch_count; ++b)
                cblas_geqrf<T>(
                    M, N, (hA.data() + b * strideA), lda, (hIpiv.data() + b * stridep), hw.data());
        }
        else
        {
            for(int b = 0; b < batch_count; ++b)
    ...
    if(argus.timing)
    {
        ...
        if(geqrf)
        {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_geqrf_strided_batched<T>(
                    handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
        ...
        {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_geqr2_strided_batched<T>(
                    handle, M, N, dA, lda, strideA, dIpiv, stridep, batch_count);
            gpu_time_used = get_time_us();
            for(int iter = 0; iter < hot_calls; iter++)
                rocsolver_geqr2_strided_batched<T>(
        ...
        cout << M << "," << N << "," << lda << "," << strideA << "," << stridep << ","
             << batch_count << "," << gpu_time_used << "," << cpu_time_used;
