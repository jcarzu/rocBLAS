...
rocblas_status testing_larfb(Arguments argus)
{
    ...
    if(N < 1 || M < 1 || K < 1 || lda < M || ldt < K || (sideC == 'L' && ldv < M)
       || (sideC == 'R' && ldv < N))
    {
    ...
    auto dV_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(T) * sizeV),
                                         rocblas_test::device_free};
    T*   dV         = (T*)dV_managed.get();
    auto dF_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(T) * sizeF),
                                         rocblas_test::device_free};
    T*   dF         = (T*)dF_managed.get();
    auto dA_managed = rocblas_unique_ptr{rocblas_test::device_malloc(sizeof(T) * sizeA),
                                         rocblas_test::device_free};
    T*   dA         = (T*)dA_managed.get();
    if(!dV || !dF || !dA)
    {
    ...
    if(argus.timing)
    {
        ...
        cout << endl;
        cout << sideC << "," << transC << "," << directC << "," << M << "," << N << "," << K << ","
             << ldv << "," << ldt << "," << lda << "," << gpu_time_used << "," << cpu_time_used;
