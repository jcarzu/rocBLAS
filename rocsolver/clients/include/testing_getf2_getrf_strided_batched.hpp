...
rocblas_status testing_getf2_getrf_strided_batched(Arguments argus)
{
    ...
    if(M < 1 || N < 1 || lda < M || batch_count < 1)
    {
        ...
        if(getrf)
        {
            return rocsolver_getrf_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, dinfo, batch_count);
        }
        else
        {
            return rocsolver_getf2_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, dinfo, batch_count);
        }
    }
    ...
    if(argus.unit_check || argus.norm_check)
    {
        ...
        if(getrf)
        {
            CHECK_ROCBLAS_ERROR(rocsolver_getrf_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, dinfo, batch_count));
        }
        else
        {
            CHECK_ROCBLAS_ERROR(rocsolver_getf2_strided_batched<T>(
                handle, M, N, dA, lda, strideA, dIpiv, stridep, dinfo, batch_count));
        }
    ...
    if(argus.timing)
    {
        ...
        if(getrf)
        {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_getrf_strided_batched<T>(
                    handle, M, N, dA, lda, strideA, dIpiv, stridep, dinfo, batch_count);
        ...
        {
            for(int iter = 0; iter < cold_calls; iter++)
                rocsolver_getf2_strided_batched<T>(
                    handle, M, N, dA, lda, strideA, dIpiv, stridep, dinfo, batch_count);
            gpu_time_used = get_time_us();
            for(int iter = 0; iter < hot_calls; iter++)
                rocsolver_getf2_strided_batched<T>(
