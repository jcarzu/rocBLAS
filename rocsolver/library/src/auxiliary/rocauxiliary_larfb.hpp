...
__global__ void copymatA1(const rocsolver_int ldw,
                          const rocsolver_int order,
                          U                   A,
                          const rocsolver_int shiftA,
...
template <typename T, typename U>
__global__ void addmatA1(const rocsolver_int ldw,
                         const rocsolver_int order,
                         U                   A,
                         const rocsolver_int shiftA,
                         const rocsolver_int lda,
...
template <typename T, typename U>
rocblas_status rocsolver_larfb_template(rocsolver_handle          handle,
                                        ...
                                        const rocsolver_int       strideV,
                                        U                         F,
                                        const rocsolver_int       shiftF,
                                        const rocsolver_int       ldf,
                                        const rocsolver_int       strideF,
                                        U                         A,
                                        const rocsolver_int       shiftA,
                                        const rocsolver_int       lda,
                                        const rocsolver_int       strideA,
                                        const rocsolver_int       batch_count)
{
    ...
    for(int b = 0; b < batch_count; ++b)
    {
        Fp = load_ptr_batch<T>(FF, b, shiftF, strideF);
        rocblas_trmm(handle,
                     side,
                     uploT,
                     trans,
