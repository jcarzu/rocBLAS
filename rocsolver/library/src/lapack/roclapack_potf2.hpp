...
template <typename T>
rocblas_status rocsolver_potf2_template(
    rocblas_handle handle, rocblas_fill uplo, rocblas_int n, T* a, rocblas_int lda)
{
    ...
    if(uplo == rocblas_fill_upper)
    {
        ...
        for(rocblas_int j = 0; j < n; ++j)
        {
            ...
            if(j > 0)
            {
                rocblas_dot<T>(handle,
                               j,
                               &a[idx2D(0, j, lda)],
                               oneInt,
                               &a[idx2D(0, j, lda)],
                               oneInt,
                               &inpsResGPU[POTF2_RESDOT]);
                hipLaunchKernelGGL(sqrtDiagOnward<T>,
                                   dim3(1),
                                   dim3(1),
                                   0,
                                   stream,
            ...
            {
                hipLaunchKernelGGL(
                    sqrtDiagFirst<T>, dim3(1), dim3(1), 0, stream, a, idx2D(j, j, lda), inpsResGPU);
            }
            ...
            if(j < n - 1)
            {
                rocblas_gemv<T>(handle,
                                ...
                                &a[idx2D(0, j, lda)],
                                oneInt,
                                &(inpsResGPU[POTF2_INPONE]),
                                &a[idx2D(j, j + 1, lda)],
                                lda);
                rocblas_scal<T>(
                    handle, n - j - 1, &inpsResGPU[POTF2_RESINVDOT], &a[idx2D(j, j + 1, lda)], lda);
            }
        }
    }
    else
        ...
        for(rocblas_int j = 0; j < n; ++j)
        {
            ...
            if(j < n - 1)
            {
                rocblas_gemv<T>(handle,
                                ...
                                &a[idx2D(j + 1, j, lda)],
                                oneInt);
                rocblas_scal<T>(handle,
                                n - j - 1,
                                &inpsResGPU[POTF2_RESINVDOT],
                                &a[idx2D(j + 1, j, lda)],
                                oneInt);
            }
        }
    ...
    hipMemcpy(&inpsResHost[POTF2_RESPOSDEF],
              &inpsResGPU[POTF2_RESPOSDEF],
              sizeof(T),
