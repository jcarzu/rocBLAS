...
inline __global__ void getrf_check_singularity(const rocblas_int  n,
                                               const rocblas_int  j,
                                               rocblas_int*       ipivA,
                                        ...
                                        const rocblas_int batch_count)
{
    ...
    if(m < GETRF_GETF2_SWITCHSIZE || n < GETRF_GETF2_SWITCHSIZE)
        return rocsolver_getf2_template<T>(
    ...
    for(int j = 0; j < dim; j += GETRF_GETF2_SWITCHSIZE)
    {
        ...
        rocsolver_getf2_template<T>(handle,
                                    m - j,
                                    jb,
                                    A,
        ...
        hipLaunchKernelGGL(getrf_check_singularity,
                           gridPivot,
                           threads,
                           0,
                           stream,
        ...
        rocsolver_laswp_template<T>(handle,
                                    j,
                                    A,
                                    shiftA,
                                    lda,
