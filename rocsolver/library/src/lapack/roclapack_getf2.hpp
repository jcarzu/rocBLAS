...
inline __global__ void getf2_check_singularity(U                 AA,
                                        ...
                                        const rocblas_int batch_count)
{
    ...
    for(rocblas_int j = 0; j < dim; ++j)
    {
        ...
        for(int b = 0; b < batch_count; ++b)
        {
            ...
            rocblas_iamax(
                handle, m - j, (M + idx2D(j, j, lda)), 1, (ipiv + shiftP + b * strideP + j));
        }

        // adjust pivot indices and check singularity
        hipLaunchKernelGGL(getf2_check_singularity<T>,
                           dim3(batch_count),
                           dim3(1),
                           0,
                           stream,
        ...
        {
            M = load_ptr_batch<T>(AA, b, shiftA, strideA);
            rocblas_scal(handle, (m - j - 1), (pivotGPU + b), (M + idx2D(j + 1, j, lda)), oneInt);
        }
