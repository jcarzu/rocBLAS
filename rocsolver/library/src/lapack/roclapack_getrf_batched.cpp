                                            ...
                                            rocblas_int       batch_count)
{
    ...
    return rocsolver_getrf_template<T>(
        handle,
        m,
        n,
        A,
...
ROCSOLVER_EXPORT rocblas_status rocsolver_sgetrf_batched(rocsolver_handle    handle,
...
{
    return rocsolver_getrf_batched_impl<float>(
        handle, m, n, A, lda, ipiv, strideP, info, batch_count);
}

ROCSOLVER_EXPORT rocblas_status rocsolver_dgetrf_batched(rocsolver_handle    handle,
                                                         const rocsolver_int m,
                                                         const rocsolver_int n,
                                                         double* const       A[],
                                                         const rocsolver_int lda,
...
{
    return rocsolver_getrf_batched_impl<double>(
        handle, m, n, A, lda, ipiv, strideP, info, batch_count);
}
