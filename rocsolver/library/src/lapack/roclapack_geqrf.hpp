                                        ...
                                        const rocblas_int batch_count)
{
    ...
    if(m <= GEQRF_GEQR2_SWITCHSIZE || n <= GEQRF_GEQR2_SWITCHSIZE)
        return rocsolver_geqr2_template<T>(
    ...
    hipLaunchKernelGGL(
        get_array, dim3(blocks), dim3(256), 0, stream, dF, work, strideW, batch_count);
    U F = U(dF);
#else
    T* F = work;
#endif
    ...
    while(j < dim - GEQRF_GEQR2_SWITCHSIZE)
    {
        ...
        rocsolver_geqr2_template<T>(handle,
                                    m - j,
                                    jb,
                                    A,
        ...
        if(j + jb < n)
        {
            ...
            rocsolver_larft_template<T>(handle,
                                        rocsolver_forward_direction,
                                        m - j,
                                        jb,
                                        A,
                                        shiftA + idx2D(j, j, lda),
            ...
            rocsolver_larfb_template<T>(handle,
                                        rocblas_side_left,
